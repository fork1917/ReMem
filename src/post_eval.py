...
def parse_dataset_files(object_name, dataset_base_dir, anomaly_maps_dir, dataset="MVTec"):
    ...
    for subdir in listdir(str(test_dir)):
        ...
        if subdir != 'good':
            _, mask_type = os.path.splitext(listdir(path.join(gt_base_dir, subdir))[0])
            if dataset != 'MVTec':
                gt_filenames.extend(
...
def eval_segmentation(gt_filenames, prediction_filenames, pro_integration_limit=0.3, delete_tiff_files=True):
    ...
    print(f"AP (pixel-level): {auroc_px}", end=" -- ")
...
def eval_classification(gt_filenames, prediction_filenames, aggregation_statistics = "meantop1p", anomaly_scores=None):
    # Read all ground truth and anomaly images.
    ground_truth = []
    predictions = []
    ...
    for (gt_name, pred_name) in tqdm(zip(gt_filenames, prediction_filenames),
                                     total=len(gt_filenames)):
        if gt_name is not None:
            tuozhan = gt_name.split(".")[-1]
        test_path.append(os.path.join(pred_name.split("/")[-2], pred_name.split("/")[-1]))
        if First_flag:
            First_flag = False
    ...
    if anomaly_scores is not None:
        for idx, path in enumerate(test_path):
            predictions[idx] = anomaly_scores[path+'.'+tuozhan]
